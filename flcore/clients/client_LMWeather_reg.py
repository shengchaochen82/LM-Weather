...
class client_LMWeather_reg(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        super().__init__(args, id, train_samples, test_samples, **kwargs)

        if args.use_reg:
            self.l2_weight = 0.01
            self.global_lora = nn.ModuleList()
            for name, param in self.named_parameters():
                if 'lora' in name:
                    self.global_lora.append(copy.deepcopy(param))
    ...
    def train(self):
        ...
        for step in range(max_local_steps):
            ...
            # Regularize the local Lora and Global Lora
            if self.args.use_reg:
                local_lora_params = nn.ModuleList()
                for name, param in self.model.named_parameters():
                    if 'lora' in name:
                        local_lora_params.append(param)
            ...
            for i, (x, y, x_mark, y_mark) in enumerate(trainloader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.use_reg:
                        ...
                        for local_param, global_param in zip(local_lora_params, self.global_lora):
                            regularizer_loss += torch.norm(local_param - global_param, 2)
                        loss += self.l2_weight * regularizer_loss
                    else:
                        loss = loss
