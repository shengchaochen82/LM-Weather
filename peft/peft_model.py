    ...
    **Attributes**:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            past_key_values = self.get_prompt(batch_size)
            # return self.base_model(input_ids=input_ids, past_key_values=past_key_values, **kwargs) -> Original
            return self.base_model(inputs_embeds=inputs_embeds, past_key_values=past_key_values, **kwargs)
        else:
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
            prompts = self.get_prompt(batch_size=batch_size)
